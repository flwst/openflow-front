      ...
      <Formik
        ...
        onSubmit={async (values: { email: string; password: string }) => {
          ...
          if (loginResponse.ok) {
            ...
            // Initialize CDP wallet after successful login
            try {
              const cdpResponse = await fetch('/api/auth/cdp-jwt', {
                credentials: 'include'
              });
              
              if (cdpResponse.ok) {
                const cdpData = await cdpResponse.json();
                console.log('CDP wallet initialized for user:', cdpData.user_id);
                sessionStorage.setItem('cdp_wallet_initialized', 'true');
              }
            } catch (error) {
              console.error('⚠️ CDP wallet initialization failed:', error);
              // Don't block login if CDP fails
            }
            // End CDP integration
